...
class FastModel:
    def __init__(self, model_name, rotary_emb: RotaryEmbedding = RotaryEmbedding()) -> None:
    ...
    def attention_fast_forward(self, layer, hidden_states, past_key_value, position_embeddings):
        ...
        K = repeat_kv(K, self.n_groups)  # (bsz, n_heads, kv_seq_len, head_dim)
        V = repeat_kv(V, self.n_groups)
    ...
    def forward(self, input_ids, labels):
        ...
        T0 = 0
        position_embedding = self.cos[:, T0:T0+seq_len], self.sin[:, T0:T0+seq_len]
